...
class DecorationEditor:
    ...
    def __init__(self, app_ref):
        self.app = app_ref
        self.font_ui = self.app.font_ui; self.font_title = self.app.font_title; self.font_desc = pygame.font.SysFont("Arial", 12)
        self.catalog_data = self.app.data_manager.load_catalog()
        self.open_main_cat_indices = set(); self.open_sub_cat_indices = set()
        self.selected_deco_item = None; self.clickable_elements = []
        self.scroll_y = 0; self.content_height = 0; self.is_scrolling_with_thumb = False
        self.scroll_start_y = 0; self.scroll_start_scroll_y = 0
        self.active_search_term = ""; self.search_input = None; self.search_button = None
        self.ghost_pos = (0, 0); self.ghost_rotation = 0
        self.panel_rect = pygame.Rect(0, 0, 0, 0); self.content_surface = None
        self.scrollbar_track_rect = None; self.scrollbar_thumb_rect = None

    def update_layout(self):
        self.panel_rect = self.app.right_panel_rect; margin, input_height = 10, 28; button_size = input_height; button_padding = 5
        self.search_button = Button(self.panel_rect.x + margin, self.panel_rect.y + margin, button_size, button_size, "", self.app.font_ui)
        input_x = self.search_button.rect.right + button_padding; input_width = self.panel_rect.width - (margin * 2) - button_size - button_padding
        ...
        scrollbar_width = 15; self.scrollbar_track_rect = pygame.Rect(self.panel_rect.right - scrollbar_width, self.panel_rect.top, scrollbar_width, self.panel_rect.height)
    ...
    def handle_events(self, event, mouse_pos, local_mouse_pos, keys):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1 and self.scrollbar_thumb_rect and self.scrollbar_thumb_rect.collidepoint(mouse_pos): self.is_scrolling_with_thumb = True; self.scroll_start_y = mouse_pos[1]; self.scroll_start_scroll_y = self.scroll_y
            ...
            if event.button == 1:
                ...
                if is_on_content: self.handle_panel_click(mouse_pos)
                elif self.selected_deco_item and self.app.editor_rect.collidepoint(mouse_pos): self.place_decoration(self.ghost_pos)
        ...
        if event.type == pygame.MOUSEMOTION:
            if self.is_scrolling_with_thumb:
                delta_y = mouse_pos[1] - self.scroll_start_y; scrollable_px_range = self.scrollbar_track_rect.height - self.scrollbar_thumb_rect.height; scrollable_content_range = self.content_height - self.panel_rect.height
                if scrollable_px_range > 0: self.scroll_y = self.scroll_start_scroll_y + delta_y * (scrollable_content_range / scrollable_px_range); self.clamp_scroll()
            if self.selected_deco_item and self.app.editor_rect.collidepoint(mouse_pos): self.ghost_pos = screen_to_grid(local_mouse_pos[0], local_mouse_pos[1], self.app.camera.offset)
        
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE: self.selected_deco_item = None
    ...
    def handle_panel_click(self, mouse_pos):
        content_y_start = self.search_input.rect.bottom + 10; local_x = mouse_pos[0] - self.panel_rect.x; local_y = mouse_pos[1] - content_y_start + self.scroll_y
    ...
    def draw_search_icon(self, screen, rect):
        center = rect.center; radius = min(rect.width, rect.height) // 4; pygame.draw.circle(screen, COLOR_TEXT, center, radius, 2); angle = math.radians(135)
        start_pos = (center[0] + radius * math.cos(angle), center[1] - radius * math.sin(angle)); end_pos = (center[0] + radius * 2 * math.cos(angle), center[1] - radius * 2 * math.sin(angle))
        pygame.draw.line(screen, COLOR_TEXT, start_pos, end_pos, 3)

    def draw_content(self):
        self.content_surface.fill((0, 0, 0, 0)); self.clickable_elements.clear()
    ...
    def draw_search_results_view(self):
        search_results = []; processed_ids = set()
    ...
    def draw_catalog_view(self):
        y_pos, margin = 0, 10; has_scrollbar = self.content_height > self.panel_rect.height; content_width = self.panel_rect.width - margin * 2 - (self.scrollbar_track_rect.width if has_scrollbar else 0)
        for i, main_cat in enumerate(self.catalog_data.get("categories", [])):
            is_open = i in self.open_main_cat_indices; rect = pygame.Rect(margin, y_pos, content_width, 28); pygame.draw.rect(self.content_surface, COLOR_BUTTON_ACTIVE if is_open else COLOR_BUTTON, rect, border_radius=5); self.clickable_elements.append({'rect': rect, 'type': 'main_cat', 'id': i})
            title = self.font_ui.render(("v " if is_open else "> ") + main_cat["name"], True, COLOR_TEXT); self.content_surface.blit(title, (rect.x + 10, rect.centery - title.get_height() // 2)); y_pos += rect.height + 5
            if is_open:
                for j, sub_cat in enumerate(main_cat["subcategories"]):
                    sub_is_open = (i, j) in self.open_sub_cat_indices; sub_rect = pygame.Rect(margin + 15, y_pos, content_width - 15, 22); pygame.draw.rect(self.content_surface, COLOR_BUTTON_HOVER if sub_is_open else (45, 55, 65), sub_rect, border_radius=3); self.clickable_elements.append({'rect': sub_rect, 'type': 'sub_cat', 'id': (i, j)})
                    sub_title = self.font_ui.render(("v " if sub_is_open else "> ") + sub_cat["name"], True, COLOR_TEXT); self.content_surface.blit(sub_title, sub_title.get_rect(center=sub_rect.center)); y_pos += sub_rect.height + 4
                    if sub_is_open and sub_cat["items"]: y_pos += self.draw_item_grid(sub_cat["items"], margin + 15, y_pos + 10, content_width - 15)
        return y_pos

    def draw_item_grid(self, items, start_x, start_y, width):
        icon_size, padding = 60, 10; cols = max(1, (width + padding) // (icon_size + padding))
        for k, item in enumerate(items):
            ...
            for word in words:
                ...
                else: surf = self.font_desc.render(line.strip(), True, COLOR_TEXT); self.content_surface.blit(surf, surf.get_rect(centerx=item_rect.centerx, top=item_name_y)); item_name_y += self.font_desc.get_linesize(); line = word + " "
            if line: surf = self.font_desc.render(line.strip(), True, COLOR_TEXT); self.content_surface.blit(surf, surf.get_rect(centerx=item_rect.centerx, top=item_name_y))
    ...
    def update_scrollbar_thumb(self):
        ...
        if self.content_height > content_visible_h:
            thumb_h = max(20, content_visible_h * (content_visible_h / self.content_height)); scroll_ratio = self.scroll_y / max(1, self.content_height - content_visible_h)
            thumb_y = self.scrollbar_track_rect.y + scroll_ratio * (self.scrollbar_track_rect.height - thumb_h); self.scrollbar_thumb_rect = pygame.Rect(self.scrollbar_track_rect.x, thumb_y, self.scrollbar_track_rect.width, thumb_h)
